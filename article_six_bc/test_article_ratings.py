...
SCORING_PROMPT = """
Review the article below and score it on a scale of 1 to 10.
...
{{
  ...
  "reasoning": "<A brief, one-sentence justification for your score.>"
}}
"""
...
def parse_score_from_response(response: dict) -> int:
    """Parses the overall 'score' from a single response JSON."""
...
def parse_reasoning_from_response(response: dict) -> str:
    """Parses the 'reasoning' from a single response JSON."""
    if "error" in response or not isinstance(response, dict):
        # Include the raw response in the reasoning if there was an error
        return f"Error in response: {response.get('raw_response', 'Unknown error')}"
...
def run_evaluation_for_model(client_or_model, display_model_name: str, all_results: list, run_dir: Path):
    ...
    model_reasoning_data = [] # To hold all reasoning data for this model
    ...
    for article_id, article_text in TEST_ARTICLES.items():
        ...
        for i in range(1, NUM_EVAL_RUNS_PER_ARTICLE + 1):
            ...
            score = parse_score_from_response(response_json)
            reasoning = parse_reasoning_from_response(response_json)

            # Store the data for this specific run
            model_reasoning_data.append({
    ...
    # Save the consolidated reasoning file for the current model
    sanitized_model_name = display_model_name.replace("/", "_").replace(":", "_")
    reasoning_file_path = run_dir / f"{sanitized_model_name}_reasoning.json"
    print(f" >> Saving all reasoning for '{display_model_name}' to {reasoning_file_path}")
    with reasoning_file_path.open("w", encoding="utf-8") as f:
        json.dump(model_reasoning_data, f, indent=2, ensure_ascii=False)
...
def main():
    ...
    run_dir = RUNS_DIR / f"benchmark_simple_{run_timestamp}"
